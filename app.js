...
function renderSettings(){content.innerHTML=`<h1 class="screen-title">PIN-Schutz</h1><p class="intro">Die PIN schützt den Zugriff auf diesem Gerät und in diesem Browser.</p><section class="panel"><label class="field"><span>Neue PIN</span><input id="new-pin" type="password" inputmode="numeric" value="${esc(state.pin)}"></label><label class="field"><span>PIN wiederholen</span><input id="repeat-pin" type="password" inputmode="numeric"></label><button id="pin-save" class="primary wide">PIN speichern</button><p class="hint">Leer lassen und speichern, um den PIN-Schutz auszuschalten.</p></section><section class="panel"><h2>Datensicherung</h2><p>Für einen Gerätewechsel exportierst du die Einträge und importierst sie auf dem anderen Gerät. Die App synchronisiert nicht automatisch über das Internet.</p></section>`;content.querySelector('#pin-save').onclick=()=>{const a=content.querySelector('#new-pin').value,b=content.querySelector('#repeat-pin').value;if(a!==b)return toast('Die PIN-Eingaben stimmen nicht überein.');state.pin=a;save();toast(a?'PIN gespeichert.':'PIN-Schutz ausgeschaltet.');};}
